...
public class AdminOrderController {
    ...
    public void orderExcelExport(HttpServletResponse response, @RequestParam Map<String, String> dateMap) throws Exception {
        ...
        for (OrderDTO orderDTO : adminOrderService.listNewOrder(condMap)) {
            ...
            cell = row.createCell(8);
            cell.setCellStyle(bodyStyle);
            if (orderDTO.getDeliveryState().equals("deliveryPrepared"))
                cell.setCellValue("배송준비중");
            if (orderDTO.getDeliveryState().equals("delivering"))
                cell.setCellValue("배송중");
            if (orderDTO.getDeliveryState().equals("finishedDelivering"))
                cell.setCellValue("배송완료");
            if (orderDTO.getDeliveryState().equals("cancelOrder"))
                cell.setCellValue("주문취소");
            if (orderDTO.getDeliveryState().equals("returningGoods"))
